...
import { useCreatePromotion } from "@/lib/queries/promotions";
import type { CreatePromotionRequest } from "@/lib/types";
...
interface PromotionCreateFormProps extends ComponentProps<"form"> {
	businessId: number;
	onSaved?: () => void;
	onCancel?: () => void;
}
...
export function PromotionCreateForm({
	...
	className,
	...props
}: Readonly<PromotionCreateFormProps>) {
	const navigate = useNavigate();
	const create = useCreatePromotion(businessId);
	...
	const form = useForm({
		defaultValues: {
			...
			validFrom: "",
			validTo: "",
			eligibleServiceIds: [] as number[],
		} satisfies CreatePromotionFormValues,
		validators: {
			onMount: createPromotionSchema,
			onChange: createPromotionSchema,
			onSubmit: createPromotionSchema,
		},
		onSubmit: async ({ value }) => {
			try {
				...
				toast.success(`Promoção ${result.name} criada.`);
				handleSaved();
			} catch (err) {
				toast.error(
					err instanceof Error && err.message
						? err.message
						: "Não foi possível criar a promoção. Tente novamente.",
				);
			}
		},
	});
							...
							<FieldSet>
								...
								<FieldGroup data-slot="checkbox-group" className="gap-3">
									{servicesQ.isLoading ? (
										...
										<p className="text-sm text-muted-foreground">
											Cadastre serviços antes de selecionar.
										</p>
									) : (
										services.map((svc) => {
